  ...
  <div class="min-h-screen pt-safe">
    ...
    <header class="px-5 pt-4 pb-3">
      <div class="flex items-center justify-between">
        <div>
          <p class="text-slate-400 text-xs font-nunito font-semibold tracking-widest uppercase mb-0.5">
            {{ greeting }}, {{ userName }}
          </p>
          <h1 class="font-nunito font-black text-2xl text-gradient leading-tight">
            Eu Reciclo
          </h1>
        </div>
        <!-- Logo pill -->
        <div class="h-11 px-4 rounded-2xl flex items-center gap-2 glow-green"
          ...
          <span class="font-nunito font-bold text-white text-xs tracking-wide">Eco</span>
        </div>
      </div>
    </header>
        ...
        <div class="relative z-10">
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              <p class="text-white/60 text-xs font-nunito font-semibold uppercase tracking-widest mb-1">
                Estimativa atual
              </p>
              <div class="text-3xl font-nunito font-black text-white leading-none truncate">
                {{ formatCurrency(totalEstimate) }}
              </div>
              <p class="text-white/50 text-xs font-nunito mt-1.5">
            ...
            <button @click="goToCalculator"
                    class="shrink-0 flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-nunito font-bold text-xs transition-all duration-150 active:scale-95 whitespace-nowrap"
          ...
          <div class="mt-4 grid grid-cols-3 gap-2">
            <div v-for="stat in miniStats" :key="stat.label"
              ...
              <div class="text-sm font-nunito font-black text-white truncate">{{ stat.value }}</div>
              <div class="text-white/45 text-xs font-nunito mt-0.5 truncate">{{ stat.label }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="px-5 mt-6">
      <div class="flex items-center justify-between mb-3">
        <h2 class="font-nunito font-bold text-sm text-slate-500 tracking-wide uppercase">Categorias</h2>
        <router-link to="/calculator" class="text-eco-600 text-xs font-nunito font-semibold">Ver todas →</router-link>
      </div>
      <div class="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <div class="font-nunito font-bold text-sm text-slate-700">{{ cat.label }}</div>
          <div class="text-slate-400 text-xs font-nunito mt-0.5">
    ...
    <section class="px-5 mt-6 mb-4">
      <div class="flex items-center justify-between mb-3">
        <h2 class="font-nunito font-bold text-sm text-slate-500 tracking-wide uppercase">Top Materiais</h2>
        <router-link to="/materials" class="text-eco-600 text-xs font-nunito font-semibold">Gerenciar →</router-link>
      </div>
      <div class="flex flex-col gap-2">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="font-nunito font-semibold text-sm text-slate-700 leading-tight truncate">{{ mat.name }}</p>
            <p class="text-slate-400 text-xs font-nunito mt-0.5 truncate">{{ mat.description }}</p>
          </div>
          <div class="text-right shrink-0">
            <p class="font-nunito font-bold text-sm text-gradient-amber">
              {{ formatCurrency(mat.pricePerKg) }}/kg
            </p>
            <p v-if="mat.unitType === 'units'" class="text-slate-400 text-xs font-nunito">
